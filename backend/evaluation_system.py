...
=============================================================
  ...
  AUTO-MODE:
    - Polls MongoDB every 10 minutes for newly added
      extractedanswers (added within the last 1 hour)
    - Groups them by base_id; the system finds questionText,
      referenceAnswer, and answerText itself from MongoDB
    - Already-evaluated base_ids are skipped automatically
    - If poll finds nothing new → asks evaluator for a manual
      base_id, evaluates it, then resumes polling
...
# ── Auto-poll settings ────────────────────────────────────────
POLL_INTERVAL_SECONDS = 600        # check MongoDB every 10 minutes
RECENT_WINDOW_SECONDS = 3600       # consider answers added in last 1 hour as "new"

MODEL_PRIORITY = [
    "llama-3.3-70b-versatile",   # ✅ replaces llama-3.1-70b-versatile
    "llama-3.1-8b-instant",      # ✅ still active
    "gemma2-9b-it",              # ✅ still active
...
class MongoDataLayer:
    ...
    def get_questions_for_base_id(self, base_id: str) -> list:
        """
        Finds all active questions belonging to base_id.
        Supports TWO storage formats used by the frontend:
          Format A: questionId = "1234-1" … "1234-5"  (regex match)
          Format B: questionId = "1234-1" stored with baseQuestionId = "1234"  ($or)
        Both formats are queried simultaneously so neither is missed.
        """
        ...
        # De-duplicate (a question may satisfy both $or arms)
        seen = {}
        for q in questions:
        ...
        # Ensure questionId ends with '-N'; if it is a bare 4-digit id,
        # synthesise the dash form so the rest of the pipeline always
        # works with "base_id-N" keys.
        normalised = []
        for q in questions:
            qid = q.get("questionId", "")
            if "-" not in qid:
                # Bare 4-digit: use questionNumber field to build the key
                num = q.get("questionNumber")
                if num:
                    q = dict(q)          # copy so we don't mutate the original
                    q["questionId"] = f"{base_id}-{num}"
                    normalised.append(q)
                    logging.info(
                        f"[MongoDataLayer] Normalised bare questionId "
                        f"'{qid}' → '{q['questionId']}'"
                    )
                else:
                    logging.warning(
                        f"[MongoDataLayer] Question '{qid}' has no dash and "
                        f"no questionNumber — skipping"
                    )
            else:
                normalised.append(q)

        # Sort by question number (the digit after the last '-')
        def _sort_key(q):
            try:
                return int(q["questionId"].rsplit("-", 1)[-1])
            except (ValueError, IndexError):
                return 99

        normalised.sort(key=_sort_key)
        logging.info(
            f"[MongoDataLayer] {len(normalised)} question(s) found for "
            f"base_id={base_id}: "
            f"{[q['questionId'] for q in normalised]}"
        )
        return normalised
    ...
    def get_answers_for_base_id(self, base_id: str) -> list:
        """
        Finds student answers for base_id.
        Primary lookup  : questionId regex  ^base_id-[1-5]$
        Fallback lookup : universalId field = "UNIV-base_id"
                          (in case questionId was stored differently)
        """
        pattern = f"^{base_id}-[1-{QUESTIONS_PER_ID}]$"
        answers = list(self.db.extractedanswers.find(
            {
                "$or": [
                    {"questionId" : {"$regex": pattern}},
                    {"universalId": f"UNIV-{base_id}"},
                ]
            },
            {"_id": 0}
        ))

        # De-duplicate and ensure every answer has a proper questionId
        seen = {}
        for a in answers:
            qid = a.get("questionId", "")
            # If questionId doesn't contain a dash, try to build it from
            # the fileId + questionNumber stored on the answer document
            if "-" not in qid:
                num = a.get("questionNumber")
                file_id = a.get("fileId", base_id)
                if num:
                    a = dict(a)
                    a["questionId"] = f"{file_id}-{num}"
                    qid = a["questionId"]
            if qid not in seen:
                seen[qid] = a
        ...
        def _sort_key(a):
            try:
                return int(a["questionId"].rsplit("-", 1)[-1])
            except (ValueError, IndexError):
                return 99

        answers.sort(key=_sort_key)
        logging.info(
            f"[MongoDataLayer] {len(answers)} answer(s) found for "
            f"base_id={base_id}: "
            f"{[a['questionId'] for a in answers]}"
        )
        return answers

    # ── NEW: find base_ids added recently (within window) ─
    def get_recent_base_ids(self, window_seconds: int = RECENT_WINDOW_SECONDS) -> list:
        """
        Scans extractedanswers for documents whose createdAt timestamp
        falls within the last `window_seconds` seconds.
        Supports both ISO-string and Python datetime createdAt values.
        Returns a sorted, deduplicated list of base_ids (the part of
        questionId before the last '-N').
        """
        cutoff = datetime.now(timezone.utc) - timedelta(seconds=window_seconds)

        # Build a query that works for both datetime objects and ISO strings.
        # MongoDB $gte works natively for datetime fields; for ISO strings we
        # filter in Python below as a fallback.
        # ── Try native datetime query first ───────────────
        # Mongoose's timestamps:true auto-populates createdAt as a Date object.
        # We try three timestamp field names in order of preference.
        recent_docs = []
        timestamp_fields = ["createdAt", "uploadDate", "updatedAt"]
        for ts_field in timestamp_fields:
            try:
                docs = list(self.db.extractedanswers.find(
                    {ts_field: {"$gte": cutoff}},
                    {"questionId": 1, "universalId": 1, "_id": 0}
                ))
                if docs:
                    recent_docs = docs
                    logging.info(
                        f"[MongoDataLayer] Poll: found {len(docs)} doc(s) "
                        f"via '{ts_field}' >= cutoff"
                    )
                    break
            except Exception as e:
                logging.debug(f"[MongoDataLayer] Poll: '{ts_field}' query failed: {e}")
                continue

        # ── Python-side fallback: parse ISO strings ────────
        if not recent_docs:
            logging.info(
                "[MongoDataLayer] Poll: native date query found nothing — "
                "falling back to Python-side timestamp parsing"
            )
            all_docs = list(self.db.extractedanswers.find(
                {},
                {"questionId": 1, "universalId": 1,
                 "createdAt": 1, "uploadDate": 1, "updatedAt": 1,
                 "_id": 0}
            ))
            for doc in all_docs:
                matched = False
                ...
                if matched:
                    recent_docs.append({
                        ...
                        "universalId": doc.get("universalId", ""),
                    })

        # ── Extract base_ids from questionId OR universalId ─
        base_ids = set()
        for doc in recent_docs:
            # Primary: derive from questionId  "1234-3" → "1234"
            ...
            # Fallback: derive from universalId  "UNIV-1234" → "1234"
            uid = doc.get("universalId", "")
            if uid.startswith("UNIV-"):
                base_ids.add(uid[len("UNIV-"):])

        result = sorted(base_ids)
        logging.info(
            f"[MongoDataLayer] {len(result)} recent base_id(s) "
            f"found in last {window_seconds}s: {result}"
        )
        return result

    # ── NEW: get all base_ids already evaluated ───────────
    def get_evaluated_base_ids(self) -> set:
        """
        Returns the set of base_ids that already have at least one
        document saved in the evaluations collection, so we don't
        re-evaluate the same group twice.
        """
        docs = list(self.db.evaluations.find({}, {"base_id": 1, "_id": 0}))
        evaluated = set()
        for doc in docs:
            bid = doc.get("base_id", "")
            if bid:
                evaluated.add(bid)
        logging.info(f"[MongoDataLayer] {len(evaluated)} already-evaluated base_id(s)")
        return evaluated
    ...
    def count_questions_for_base_id(self, base_id: str) -> int:
        """
        Counts active questions using the same dual-format $or query
        as get_questions_for_base_id so the gate check is consistent.
        """
        pattern = f"^{base_id}-[1-{QUESTIONS_PER_ID}]$"
        ...
        logging.info(
            f"[MongoDataLayer] count_questions_for_base_id({base_id}) = {count}"
        )
        return count

    # ── calibration note (Learning Agent) ─────────────────
...
# ════════════════════════════════════════════════════════════
#  AUTO-POLL LOOP  ← replaces the manual base_id input loop
# ════════════════════════════════════════════════════════════

def _run_single_base_id(base_id        : str,
                        data_layer     : MongoDataLayer,
                        orchestrator   : "OrchestratorAgent",
                        source_label   : str = "AUTO") -> bool:
    """
    Validates a base_id (questions exist + pairs matched) and hands it
    to the Orchestrator.  Returns True on success, False on skip/error.
    Used by both auto-poll and manual-fallback paths.
    """
    q_count = data_layer.count_questions_for_base_id(base_id)
    if q_count == 0:
        print(f"  ⚠️   [{source_label}] No active questions found for "
              f"base_id={base_id}. Skipping.\n")
        return False

    pairs = data_layer.get_matched_pairs(base_id)
    if not pairs:
        print(f"  ⚠️   [{source_label}] No matched question-answer pairs for "
              f"base_id={base_id}. Skipping.\n")
        return False

    print(f"  ✅  [{source_label}] {len(pairs)} matched pair(s) found. "
          f"Handing to Orchestrator...\n")
    try:
        orchestrator.run_session(base_id)
        return True
    except Exception as exc:
        logging.error(
            f"[{source_label}] Orchestrator error for base_id={base_id}: {exc}",
            exc_info=True,
        )
        print(f"  ❌  Error evaluating base_id={base_id}: {exc}\n")
        return False


def _manual_fallback(data_layer  : MongoDataLayer,
                     memory      : FeedbackMemory,
                     orchestrator: "OrchestratorAgent") -> None:
    """
    Called when the 10-minute poll finds no new answers.
    Prompts the evaluator for a 4-digit base_id, evaluates it,
    then returns so the poll loop can resume.
    """
    LINE = "═" * 65
    THIN = "─" * 65
    print(f"\n{LINE}")
    print("  ⏰  POLL WINDOW EMPTY  —  No new answers found in the last "
          f"{RECENT_WINDOW_SECONDS // 60} min")
    print("  📝  MANUAL FALLBACK  —  Enter a base_id to evaluate now,")
    print("      or press Enter to skip and resume polling.")
    print(f"{THIN}")

    while True:
        try:
            raw = input("  Base ID (4 digits) or Enter to skip: ").strip()
        except EOFError:
            raw = ""

        if raw == "":
            print("  ↩️   Skipped. Resuming poll loop...\n")
            return

        if len(raw) == 4 and raw.isdigit():
            print(f"\n{'═' * 65}")
            print(f"  🔧  MANUAL EVALUATION  base_id = {raw}")
            print(f"{'═' * 65}")
            _run_single_base_id(
                base_id      = raw,
            ...
            print(f"  ✅  Manual evaluation done. Resuming poll loop...\n")
            return
        else:
            print("  ❌  Enter exactly 4 digits, or press Enter to skip.")


def auto_poll_and_evaluate(
    data_layer   : MongoDataLayer,
    memory       : FeedbackMemory,
    ...
    """
    Continuously polls MongoDB every POLL_INTERVAL_SECONDS (10 min).

    On each tick:
      1. Find extractedanswers added within the last RECENT_WINDOW_SECONDS
         (1 hour).
      2. Group them by base_id.
      3. Skip any base_id already present in the evaluations collection.
      4. For each new base_id: verify questions + reference answers exist
         in DB, then hand off to orchestrator.run_session() exactly as
         before — the orchestrator finds questionText, referenceAnswer,
         and answerText itself from MongoDB.
      5. If nothing new is found → prompt evaluator for a manual base_id,
         evaluate it, then resume polling.

    Press Ctrl+C to stop.
    """
    LINE = "═" * 65
    print(f"\n{LINE}")
    print("  🤖  AUTO-POLL MODE  —  searches DB every "
          f"{POLL_INTERVAL_SECONDS // 60} min")
    print(f"  🕐  Detection window : last {RECENT_WINDOW_SECONDS // 60} min (1 hour)")
    print(f"  📋  On empty poll    : asks for manual base_id, then resumes")
    ...
    while True:
        ...
        print(f"  📦  Memory: {memory.count()} correction(s) loaded")
        print(f"{'─' * 65}")

        # ── Step 1: find base_ids with answers added in last 1 hour ──
        recent_base_ids = data_layer.get_recent_base_ids(
            window_seconds=RECENT_WINDOW_SECONDS
        )

        if not recent_base_ids:
            print(f"  💤  No answers found in the last "
                  f"{RECENT_WINDOW_SECONDS // 60} min.")
            _manual_fallback(data_layer, memory, orchestrator)
            # After manual fallback (or skip), sleep then poll again
            print(f"  ⏳  Next auto-poll in {POLL_INTERVAL_SECONDS // 60} min...\n")
            time.sleep(POLL_INTERVAL_SECONDS)
            continue

        # ── Step 2: skip already-evaluated base_ids ───────
        already_evaluated = data_layer.get_evaluated_base_ids()
        pending = [b for b in recent_base_ids if b not in already_evaluated]

        if not pending:
            print(f"  ✅  {len(recent_base_ids)} recent base_id(s) found but "
                  f"all already evaluated.")
            _manual_fallback(data_layer, memory, orchestrator)
            print(f"  ⏳  Next auto-poll in {POLL_INTERVAL_SECONDS // 60} min...\n")
            time.sleep(POLL_INTERVAL_SECONDS)
            continue

        print(f"  📋  {len(pending)} new base_id(s) to evaluate: {pending}")

        # ── Step 3: evaluate each pending base_id ─────────
        for base_id in pending:
            print(f"\n{'═' * 65}")
            print(f"  🚀  AUTO-EVALUATING  base_id = {base_id}")
            print(f"{'═' * 65}")
            _run_single_base_id(
                base_id      = base_id,
                data_layer   = data_layer,
...
# ════════════════════════════════════════════════════════════
#  MAIN  —  thin bootstrap, auto-poll replaces manual loop
# ════════════════════════════════════════════════════════════
...
def main():
    ...
    print("       AUTO-POLL every 10 min  |  window: last 1 hour")
    ...
    # ── Start auto-poll loop (replaces manual input loop) ─
    try:
        auto_poll_and_evaluate(
            data_layer   = data_layer,
            memory       = memory,
            agent        = agent,
            qa_agent     = qa_agent,
